...
program
  ...
  .action(async (slug: string, commandOptions: { fix?: string }) => {
    await withCliErrors(async () => {
      ...
      const baseMetadata: FrontMatterRecord = parsed ? { ...parsed.metadata } : {};
      ...
      const updatedMetadata: FrontMatterRecord = {
        ...baseMetadata,
      ...
      if (missingPhases.length > 0) {
        console.log(
          `Phase(s) ${missingPhases.join(', ')} not found in WORKFLOW.md. Running catalog build with --update-workflow.`,
        );
      }
...
interface PromptMetadataUpdate {
  id: string;
  title: string;
  path: string;
  phase: string;
...
async function resolvePromptFile(repoRoot: string, slug: string, overridePath?: string): Promise<PromptFileInfo> {
...
async function collectMetadataInteractively(
  ...
  try {
    ...
    const status = await promptForString(
      rl,
      'Status',
...
async function promptForPhases(
  rl: PromptInterface,
  defaults: string[],
  validPhases: Set<string>,
): Promise<string[]> {
...
async function promptForList(rl: PromptInterface, label: string, defaults: string[]): Promise<string[]> {
  while (true) {
...
async function updatePromptMetadataFile(repoRoot: string, update: PromptMetadataUpdate): Promise<boolean> {
...
async function runValidationScripts(repoRoot: string, updateWorkflow: boolean): Promise<void> {
...
async function runNpmScript(repoRoot: string, script: string, extraArgs: string[] = []): Promise<void> {
...
function orderMetadataEntry(entry: Record<string, unknown>): Record<string, unknown> {
...
function ensureMarkdownExtension(slug: string): string {
