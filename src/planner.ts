...
const normaliseNode = (value: unknown, index: number): GraphNode => {
  if (!isRecord(value)) {
...
const normaliseDocument = (value: unknown): GraphDocument => {
  if (!isRecord(value)) {
  ...
  return { nodes } satisfies GraphDocument;
};
...
export class Planner {
  ...
  constructor(graphPath: string, stateStore: StateStore) {
