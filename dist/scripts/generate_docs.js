...
export async function generateDocs(repoRoot, catalog, options) {
...
export async function synchronizeWorkflowDoc(repoRoot, catalog) {
...
function locateTableBounds(lines, startIndex) {
...
function buildPhaseTable(entries) {
    const rows = entries
        .slice()
...
function collectCrossEntries(catalog, usedPhases) {
    const entries = [];
    const seen = new Set();
...
function buildCrossTable(entries) {
    const rows = entries.map((entry) => `| ${escapeCell(entry.command)} | ${escapeCell(entry.stageTieIn)} | ${escapeCell(entry.purpose)} |`);
    return ['| Command | Stage tie-in | What it does |', '| --- | --- | --- |', ...rows];
}
function escapeCell(value) {
    return value.replace(/\|/g, '\\|');
}
function tableEquals(existing, replacement) {
...
export async function regenerateWorkflow(repoRoot, catalog) {
...
function buildWorkflowGraph(catalog) {
...
function renderPhaseSubgraph(phaseKey, label, commandAssignments) {
    const commands = [];
...
function buildEdges(commandLookup) {
    const edges = [];
    const seen = new Set();
...
function parsePhaseSections(lines) {
    const sections = [];
    let current = null;
    let manualBuffer = [];
...
function resolvePhaseHeading(key, bucket) {
    if (bucket.length > 0) {
...
function createManualStub(label) {
...
function buildCommandLines(entries) {
    if (entries.length === 0) {
