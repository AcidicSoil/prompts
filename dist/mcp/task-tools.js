...
const serializeTask = (task) => JSON.parse(JSON.stringify(task));
const buildTextContent = (message) => [{ type: 'text', text: message }];
export const buildTaskToolDefinitions = ({ service, logger }) => {
    ...
    const setTaskStatusTool = {
        ...
        description: 'Update the canonical status for a task. Persistence occurs only when the server write mode is enabled.',
        inputSchema: z
            .object({
            id: idSchema,
            status: statusSchema
        ...
        handler: async (rawArgs) => {
            ...
            try {
                ...
                return {
                    content: `Failed to update task ${parseResult.data.id}: ${error.message}`,
                    isError: true
                };
            }
...
export const registerTaskTools = (server, options) => {
