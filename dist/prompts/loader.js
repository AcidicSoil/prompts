...
const resolveMetadataPath = ({ metadataPath, baseDir } = {}) => {
...
export function loadPromptMetadata(options = {}) {
    ...
    try {
        ...
        throw new Error(`Failed to read prompt metadata at ${absolutePath}: ${error.message}`);
    }
    let parsed;
    try {
        ...
        throw new Error(`Failed to parse prompt metadata at ${absolutePath}: ${error.message}`);
    }
...
export function preparePromptResources(definitions, options = {}) {
    ...
    return definitions.map((definition) => {
        ...
        try {
            ...
            throw new Error(`Failed to read prompt file for \"${definition.id}\": ${error.message}`);
        }
