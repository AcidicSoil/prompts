...
export const createRunTaskActionTool = (service) => ({
    ...
    async handler(raw) {
        ...
        if (metadata && typeof metadata === 'object' && metadata.action && typeof metadata.action === 'object') {
            const record = metadata.action;
            if (typeof record.script === 'string') {
                action = { script: record.script, args: Array.isArray(record.args) ? record.args : [] };
            }
        }
        ...
        if (!action) {
            ...
            try {
                ...
                if (entry)
                    action = { script: entry.script, args: entry.args };
            }
            catch {
        ...
        const result = await runScript.handler({
